...
# from ui.doi_components import render_doi_validator, render_doi_extractor
...
if 'active_tab' not in st.session_state:
    st.session_state.active_tab = "Citation Analysis"
...
def render_homepage(settings: Settings):
    ...
    with col2:
        ...
        # Main tabs
        tab1, tab2, tab3 = st.tabs(["Citation Analysis", "DOI Validator", "Extract DOIs"])
        
        with tab1:
            render_citation_analysis_tab(settings)
        
        with tab2:
            render_doi_validator()
        
        with tab3:
            render_doi_extractor()

def render_citation_analysis_tab(settings: Settings):
    """Render the citation analysis tab"""
    # API Key input if not set
    if not st.session_state.api_key:
        st.markdown("""
        <div style="text-align: center; margin-bottom: 1rem; margin-top: 2rem;">
            <p style="color: var(--text-secondary); font-size: 16px;">Enter your Gemini API key to get started</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="footer-info">
            <p><strong>Supported Styles:</strong> APA • MLA • Chicago • Harvard • IEEE</p>
            <p><strong>Features:</strong> Format Validation • DOI Verification • Citation Discovery • Style Conversion</p>
            <p><strong>Models:</strong> 8 AI models with automatic fallback for uninterrupted service</p>
        </div>
        """, unsafe_allow_html=True)
